...
def create_pipe():
    r = random.choice(pipe_height)
    bottom_pipe = pipe_surface.get_rect(midtop=(350, r))
    top_pipe = pipe_surface.get_rect(midbottom=(350, r-150))
...
def draw_pipe(pipes):
    for p in pipes:
        if p.bottom >= 512:
            ...
            screen.blit(pygame.transform.flip(pipe_surface, False, True), p)
...
def score_display(game_state):
    ...
    if game_state == "game_over":
        ...
        high_score_rect = score_surface.get_rect(center=(95, 425))
...
screen = pygame.display.set_mode((288,515))
...
while True:
    ...
    if game_active:
        ...
        rotates_bird = rotate_bird(bird_surface)
        bird_rect.centery += bird_movement
        screen.blit(rotates_bird,bird_rect)
